 ...
 */
#define ATTACK_FILE_LENGTH 10000
#define TASKBUFSIZ	65536	// Size of task_t::buf
#define FILENAMESIZ	256	// Size of task_t::filename
#define MAXIMUM_FILE_SIZE 100000
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	//fixed2: security hole buffer overrun
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	//Jingyu: Part 3 attack 2
	if (evil_mode == 1)
	{
		//a huge file name is sent to the client of
		//other OSP2P instead of original request
		//without buffer overrun protection, the request
		//will corrupt the other hosts' stack
	       char buffer_attack_name[ATTACK_FILE_LENGTH];
	       memset(buffer_attack_name, 'f', ATTACK_FILE_LENGTH);
	       buffer_attack_name[ATTACK_FILE_LENGTH-1] = '\0';
	       while(1)
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		if (t->total_written > MAXIMUM_FILE_SIZE) {
			error("* Jingyu: File exceeds maximum file size");
			goto try_again;
		}
	}
	...
	if (t->total_written > 0) {
	    ...
	    while ((bytes = fread(data, 1, 1024, inFile)) != 0)
	        md5_append(&md_data, data, bytes);
	    md5_finish_text(&md_data, c, 1);
	    fclose(inFile);
	   
		// Get MD5 for file from tracker
		...
		*s2 = 0;
		...
		// PRINT BOTH MD5 TO TERMINAL
		printf("LOCAL  MD5: %s FOR DOWNLOADED FILE: %s\n", c, filename);
		printf("SERVER MD5: %s FOR REMOTE     FILE: %s\n", tracker_task->buf, filename);

		// COMPARE MD5
		if (strcmp(c, tracker_task->buf) == 0) {
			printf("%s %s\n", "MD5SUM match. Integrity Checked.", t->filename);
		} else {
			printf("%s %s\n", "MD5SUM don't match! Try another peer.", t->filename);
			goto try_again;
		}
		//*****
...
static void task_upload(task_t *t)
{
	...
	//**********************************************************
	// proctect against buffer overrun
	if (strlen(t->buf) > FILENAMESIZ) {
		error("* Jingyu: FILE NAME IS TOO LONG\n");
		goto exit;
	}
	...
	//**********************************************************
	//check if file name is trying to access other directory
	if (t->filename[0] == '/' || ((t->filename[0] == '~') &&(t->filename[1] == '/' || t->filename[1] == '\0')))
	{
		error("* Jingyu: Directory Denied!\n");
		goto exit;
	}
...
// part 3 attack 1
static void endless_uploads(task_t* t)
{
	assert(t->type == TASK_UPLOAD);
	// First, read the request from the peer.
	while (1) {
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	{
		pid_t pid = fork();
		if (pid < 0) //fail creating
		{
			error("ERROR: FORK FAILED\n");
			exit(0);
		}
		else if(pid == 0)//child do the task
		{
			if(evil_mode == 1)
			{
				endless_uploads(t);
